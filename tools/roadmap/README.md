...
- **代理店ごとのアクセス制御（`?agency=<トークン>`）**：`?agency=`無しの通常URLは全代理店・全メンバーが見える管理者ビュー（従来通り）。`?agency=<代理店トークン>`付きで開くと、`public/data/roadmap-<トークン>.json`（その代理店の部分木のみ）を取得し、他代理店には一切フォールバックしない。個人スタジオ側も対応する`public/studio/<トークン>.html`（その代理店のメンバーだけがEMBEDDEDに入った専用ファイル）を使う。生成は`tools/roadmap/sync_notion_to_json.py`（部分木の切り出し）→`tools/goalmap/build_agency_studio.py`（専用スタジオHTML生成）の順。詳細な運用・トークン一覧は[`ゴールマップ運用.md`§5](../../ゴールマップ運用.md)。
